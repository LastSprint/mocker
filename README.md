...
- Итеративные ответы - если для определенного URL существует несоклько моков, то они будут возвращаться по очереди. Если нет матчинга по параметрам
...
- Если вы указали несущствующую операцию, то мок **сматчится**
...
На самом деле внутри шаблона можно пистаь что угодно, но мы **настоятельно** рекомендуем дублировать имя переменной, потому что в дальнейшем эта функциональность будет расширяться.

### `requestHeaders`

Если запрос содержит те же самые хедеры, что указаны в этом поле, то мок будет сматчен. 

Мок будет считать сматченым, только если выполнены все условия матчинга (query, request, headers)
...
Пример использования: когда у вас есть мок с заданным телом запроса и вы хоитте чтоб он возвращался только в том случае, когда в запросе пришло именно это тело. 
...
- `MOCKER_LOG_PATH` - путь да файла в который `Mocker` должен писать логи. Логи пишутся в формате JSON.

## Как установить и начать использовать

В корне репоитория содержатся следующий файлы:
- `docker-compose.yaml` - содержит все необходимую конфигурацию с готов к запуску - `docker-compose up -d`
...
Во время запуска композа могут возникнуть ошибки `you try to mount directory to file (or visa versa)`. Если вы ее поймали - создайте вручную нужный файлы
